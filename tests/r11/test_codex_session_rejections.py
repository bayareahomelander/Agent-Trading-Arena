"""R11: missing, corrupt, mismatched, or ambiguous sessions never fall back."""
...
from arena_runtime.adapters.codex import CodexSessionError, CodexSessionStore
...
def _run_invocations(workspace: Path) -> list[list[str]]:
    ...
    return [
        ...
        if "exec" in item and item != ["exec", "--help"]
    ]
...
def test_mismatched_request_session_is_rejected(tmp_path: Path) -> None:
    adapter, workspace, _, store = _single(tmp_path)
    store.save("codex-product", "codex-product-1", "thread-a")
    request_with_wrong_ref = request(
        workspace,
        "codex-product-1",
        round_id="2026-08-17-late",
        prompt=LATE_PROMPT,
        session_reference="thread-b",
    )
...
def test_invalid_fresh_thread_event_is_not_persisted(
    ...
    adapter, workspace, _, store = _single(tmp_path, thread_event_mode=mode)
    fresh = request(
        workspace,
        "codex-product-1",
...
def test_resumed_output_must_report_same_thread(tmp_path: Path) -> None:
    ...
    scenario_payload["resume_thread_id"] = "different-thread"
...
def test_store_never_replaces_replica_session(tmp_path: Path) -> None:
    store = CodexSessionStore(tmp_path / "sessions")
    store.save("product", "replica", "thread-a")

    with pytest.raises(CodexSessionError) as exc:
        store.save("product", "replica", "thread-b")

    assert exc.value.path == "session_reference"


def test_one_thread_cannot_be_shared_by_two_replicas(tmp_path: Path) -> None:
    store = CodexSessionStore(tmp_path / "sessions")
    store.save("product", "replica-a", "shared-thread")

    with pytest.raises(CodexSessionError) as exc:
        store.save("product", "replica-b", "shared-thread")
