"""R16: mapped adapter results retain sanitized source artifacts."""
...
def test_quota_result_retains_explicit_code_and_source_artifact(tmp_path: Path) -> None:
    ...
    assert adapter.last_classification.error_codes == ("usage_limit_reached",)
    stdout_path = next(
        path for path in result.artifact_references if path.endswith(".jsonl")
    )
    assert "usage_limit_reached" in (archive.root / stdout_path).read_text(
        encoding="utf-8"
    )
