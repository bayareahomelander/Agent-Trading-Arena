"""R13: preflight never reads or archives Grok credential-cache contents."""
...
def test_synthetic_auth_cache_secret_never_reaches_archive(tmp_path: Path) -> None:
    ...
    assert isinstance(adapter, GrokBuildAdapter)
    credential_store = tmp_path / "grok-credentials"
...
def test_grok_preflight_source_does_not_open_auth_cache() -> None:
    module = Path(GrokBuildAdapter.__module__.replace(".", "/") + ".py")
