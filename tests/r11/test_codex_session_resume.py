"""R11: two replicas capture and resume only their own Codex sessions."""

import json
from pathlib import Path

from arena_runtime.adapters.codex import CodexSessionStore
...
def _run_commands(workspace: Path) -> list[list[str]]:
    ...
    return [entry for entry in entries if "exec" in entry and entry != ["exec", "--help"]]


def test_two_replicas_resume_exact_one_to_one_sessions(tmp_path: Path) -> None:
    replica_ids = ("codex-product-1", "codex-product-2")
    registration = make_registration(replica_ids)
    store = CodexSessionStore(tmp_path / "runtime-state" / "sessions")
    ...
    assert a_first.session_reference == "thread-a"
    assert b_first.session_reference == "thread-b"
    ...
    assert adapter_a.run(a_late).session_reference == "thread-a"
    assert adapter_b.run(b_late).session_reference == "thread-b"
    ...
    assert "resume" in a_commands[1] and "thread-a" in a_commands[1]
    assert "resume" in b_commands[1] and "thread-b" in b_commands[1]
    assert all("thread-b" not in command for command in a_commands)
    assert all("thread-a" not in command for command in b_commands)
    assert "--last" not in a_commands[1]
    assert "--last" not in b_commands[1]
...
def test_session_records_live_outside_both_replica_workspaces(tmp_path: Path) -> None:
    ...
    record = store.record_path("codex-product", "codex-product-1")
    assert record.is_file()
    assert not record.is_relative_to(workspace)
    payload = json.loads(record.read_text(encoding="utf-8"))
    assert payload["session_reference"] == "thread-a"
