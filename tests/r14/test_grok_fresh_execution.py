"""R14: fresh Grok Build argv, cwd, prompt, output, and decision are exact."""
...
def test_fresh_headless_uses_frozen_argv_cwd_and_prompt_bytes(tmp_path: Path) -> None:
    ...
    assert "-r" not in capture["argv"]
    assert "-c" not in capture["argv"]
...
def test_provider_streaming_json_and_stderr_are_sanitized_archive_artifacts(
    tmp_path: Path,
) -> None:
    ...
    stderr_path = next(
        path for path in result.artifact_references if path.endswith(".log")
    )
    stdout = (archive.root / stdout_path).read_text(encoding="utf-8")
    stderr = (archive.root / stderr_path).read_text(encoding="utf-8")

    assert '"type": "text"' in stdout
    assert '"type": "end"' in stdout
    assert "fresh-grok-session" in stdout
