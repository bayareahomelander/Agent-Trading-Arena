"""R13: documented Grok Build diagnostics prove a ready subscription runner."""
...
from .conftest import EXPECTED_MODEL, EXPECTED_REASONING, EXPECTED_VERSION, make_case


def test_ready_preflight_proves_frozen_grok_build_capabilities(tmp_path: Path) -> None:
    ...
    assert capabilities.authentication_mode == "grok.com"
    ...
    assert capabilities.headless_prompt is True
    assert capabilities.json_output is True
    assert capabilities.streaming_json is True
    assert capabilities.acp_stdio is True
...
def test_ready_summary_contains_only_safe_capability_metadata(tmp_path: Path) -> None:
    ...
    assert summary["capabilities"]["authentication_mode"] == "grok.com"
